...
2025-02-04: TODO: Save this webpage to Zotero then update citation
 -->
{{< bibliography cited >}}
...
Evolutionary game theory uses the [[wp>replicator equation]] to formalize the dynamics of selected heritable traits in a population.  It is frequently used to predict the [[wp>evolutionarily stable state]] of an infinite, well-mixed population.  Often it is interesting to study cases where the population is finite or not well-mixed, where the replicator equation isn't suitable.  Such a case is better suited to an [[wp>agent-based model]] where each individual in a population is explicitly accounted.  

There are a variety of approaches to agent-based modeling but I am keen on [[wp>reaction kinetics]] because it is simple to understand, easy to apply, and behaves predictably in the limit of a large, well-mixed population.  Also, the underlying mechanisms have clear interpretations if the reactions are [[wp>elementary reaction| elementary]].  

It would be useful to have a reaction-kinetic framework for game theory and the replicator equation.  It could complement [[wp>Moran process|other agent-based formalisms]] for studying finite, structured populations.  Below I will show a reaction kinetics approach to game theory that captures some important properties of the replicator equation.

I have three goals:
  - Selection should act in a [[wp>replicator equation]]-like manner, with the same behaviour at least up to a time-rescaling,
  - In the absence of selection, the density should obey the [[wp>logistic equation]], and
  - The reactions should be simple, preferably [[wp>elementary reaction| elementary]].
...
In the following sections we will explore several ways to define birth-death reactions, analyzing them to see how well they meet the above goals.  
  ...
  * [[#Select births, ecological deaths]]
  * [[#Replacement]]
  * [[#Select deaths, ecological births]]
  * [[#Select births with a separate game]]
  * [[#Select births into available "holes"]]
...
\begin{eqnarray}
X_i + X_j & \xrightarrow{P_{ij}} & 2 X_i + X_j   & \text{(birth)} \\
...
These two processes can be applied to simulate a finite population, or analyzed in the limit of an infinite, well-mixed population.  Then we find the dynamics are very similar to the replicator equation and reproduce some important properties thereof.  The [[wp>Law of mass action]] allows to compute the rate equation for the density \(x_i=[X_i]\) of type \(i\):
\begin{equation}
...
\begin{eqnarray}
  ...
  \label{eq:dyidt} \\
  & = & n \left[ y_i \left( \sum_j P_{ij} y_j - \frac{1}{n} \delta(n) \right) - y_i \left( \sum_{jk} P_{kj} y_j y_k - \frac{1}{n} \delta(n) \right) \right] \\  
  & = & n y_i (f_i - \bar{f}).
...
\end{equation}
where $K \equiv r n^2 / \delta(n)$.  If $\delta(n) \propto n^2$ then $K$ is a constant carrying capacity.  The dynamics are similar to logistic except the quadratic factor depresses the growth rate when the population is small, a [[wp>Allee_effect#Strong_vs._weak_Allee_effects | weak Allee effect]].
...
<WRAP center round box 90%>
\begin{eqnarray}
X_i & \xrightarrow{\beta(n)} & 2 X_i         & \text{(birth)} \\
...
<WRAP center round box 90%>
\begin{eqnarray}
  Z_{ik} + Z_{jl} & \xrightarrow{\gamma} & Z_{ij} + Z_{jl} & \text{(game)} \\
  Z_{ij}          & \xrightarrow{P_{ij}} & 2 Z_{ij}        & \text{(birth)} \\
...
In terms of the frequencies $w_{ij} \equiv z_{ij}/n$ and $y_i \equiv x_i/n = \sum_j w_{ij}$, after some work (see [[#Appendix]]) we find
\begin{equation}
...
\begin{eqnarray}
  ...
  \label{eq:separategamereplicator} \\
...
<WRAP center round box 90%>
\begin{eqnarray}
X_i + X_j + H & \xrightarrow{P_{ij}} & 2 X_i + X_j   & \text{(birth)} \\
...
^  System                                   ^  Processes((Reactions written in tems of preferred elementary reactions.  All rates parameters are constant, independent of population density or structure.))                                                                                                      ^  Replicator-like?  ^  Ecology((In the absence of selective pressure, $P_{ij}=\text{constant}$.))  ^
|  [[#Select births, ecological deaths]]    |  \begin{eqnarray*} X_i + X_j & \xrightarrow{P_{ij}} & 2 X_i + X_j   & \text{(birth)} \\ X_i + 2 X & \xrightarrow{\delta} & 2 X. & \text{(death)}  \end{eqnarray*}                                                                                                   |  Yes               |  Logistic with weak Allee effect                                             |
|  [[#Replacement]]                         | $$ X_i + X_j + X_k \xrightarrow{P_{ij}} 2 X_i + X_j $$                                                                                                                                                                                                              |  Yes               |  None (constant population size)                                             |
|  [[#Select deaths, ecological births]]    | \begin{eqnarray*} X_i & \xrightarrow{\beta} & 2 X_i         & \text{(birth)} \\ X_i + X_j & \xrightarrow{\pi - P_{ij}} & X_j & \text{(death)}  \end{eqnarray*}                                                                                                      |  Yes               |  Logistic                                                                    |
|  [[#Select births with a separate game]]  | \begin{eqnarray*}   Z_{ik} + Z_{jl} & \xrightarrow{\gamma} & Z_{ij} + Z_{jl} & \text{(game)} \\   Z_{ij}          & \xrightarrow{P_{ij}} & 2 Z_{ij}        & \text{(birth)} \\   Z_{ik} + Z_{jl} & \xrightarrow{\delta} & Z_{jl} & \text{(death)}  \end{eqnarray*}  |  Yes               |  Logistic                                                                    |
...
\begin{eqnarray}
  \frac{dz_{ij}}{dt} & = & \gamma \sum_{kl} ( z_{ik} z_{jl} - z_{ij} z_{kl} ) + P_{ij} z_{ij} - \delta(n) z_{ij} \\
  & = & \gamma ( x_i x_j - z_{ij} n ) + (P_{ij} - \delta(n)) z_{ij}.
  \label{eq:dzijdt}
\end{eqnarray}

\begin{eqnarray}
  \frac{dw_{ij}}{dt} & = & \frac{1}{n} \left[ \frac{dz_{ij}}{dt} - w_{ij} \frac{dn}{dt} \right] \\
  \frac{dx_i}{dt} & = & \sum_j \frac{dz_{ij}}{dt} \\
    & = & \gamma (x_i n - n x_i) + \sum_j P_{ij} z_{ij} - \delta(n) x_i \\
    & = & \sum_j P_{ij} z_{ij} - \delta(n) x_i. \\
  \frac{dn}{dt} & = & \sum_i \frac{dx_i}{dt} \\
    & = & n \left( \sum_{ij} P_{ij} w_{ij} - \delta(n) \right).
\end{eqnarray}
From Equation $\ref{eq:dzijdt}$
\begin{eqnarray}
  \frac{dz_{ij}}{dt} & = & \gamma n^2 ( y_i y_j - w_{ij} ) + n (P_{ij} - \delta(n)) w_{ij} \\
  \frac{dw_{ij}}{dt} & = & \frac{1}{n} \left[ \frac{dz_{ij}}{dt} - w_{ij} \frac{dn}{dt} \right] \\
                     & = & \gamma n (y_i y_j - w_{ij}) + (P_{ij} - \delta(n)) w_{ij} - w_{ij} \sum_{kl} P_{kl} w_{kl} + \delta(n) w_{ij} \\
  \frac{dy_i}{dt}    & = & \sum_j P_{ij} w_{ij} - y_i \sum_{kl} P_{kl} w_{kl}.
\end{eqnarray}
